...
const STATUS_LABELS: Record<string, { label: string; color: string }> = {
  ...
  PROCESSING: { label: 'Processing', color: 'bg-blue-50    text-blue-700   ring-1 ring-blue-200'  },
  READY:      { label: 'Ready',      color: 'bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200' },
...
export default function KnowledgeBaseGoogleDriveRow({ doc, isSelected, onToggleSelect, onDeleteDocument, onSkipDocument, onRetryDocument }: Readonly<DocumentRowProps>) {
  ...
  const rowActions = (
    <>
      {isActive && (
        <button type="button" onClick={() => onSkipDocument(doc.id)} className="rounded p-1.5 text-muted-foreground hover:bg-amber-50 hover:text-amber-600 transition-colors" title="Skip">
          <SkipForward className="h-3.5 w-3.5" />
        </button>
      )}
      {isError && (
        <button type="button" onClick={() => onRetryDocument(doc.id)} className="rounded p-1.5 text-muted-foreground hover:bg-primary/10 hover:text-primary transition-colors" title="Retry">
          <RotateCcw className="h-3.5 w-3.5" />
        </button>
      )}
      <a href={`https://drive.google.com/file/d/${doc.driveId}`} target="_blank" rel="noopener noreferrer" className="rounded p-1.5 text-muted-foreground hover:bg-muted hover:text-foreground transition-colors" title="Open in Drive">
        <ExternalLink className="h-3.5 w-3.5" />
      </a>
      <button type="button" onClick={() => onDeleteDocument(doc.id)} className="rounded p-1.5 text-muted-foreground hover:bg-red-50 hover:text-red-600 transition-colors" title="Delete">
